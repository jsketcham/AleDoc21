...
//                text = "Playback\npending"
    ...
    @objc func progressBarService(){
        ...
        // 8/22/24, multiple progress bars for merged cues
        ...
        let progressBarPerCue = UserDefaults.standard.bool(forKey: "progressBarPerCue")
        ...
        if aleDelegate?.cycleMotion == CYCLE_MOTION_ACTIVE,
           ...
           first != NSNotFound,
           last != NSNotFound,
           enProgressBar && !(isPlaybackMode && inhStreamerInPb){
            
                // progress bar for each cue?
                
                if progressBarPerCue{
                    
                    for i in first...last{
                        
                         if let dict : NSMutableDictionary = doc.tableContents.object(at: i) as? NSMutableDictionary,
                            let start = tcf?.tc(for: dict["Start"] as? String),
                            let end = tcf?.tc(for: dict["End"] as? String),
                ...
                // progress bar from start of first merged cue to end of last merged cue
                
                 if let dict : NSMutableDictionary = doc.tableContents.object(at: first) as? NSMutableDictionary,
                    let dict2 : NSMutableDictionary = doc.tableContents.object(at: last) as? NSMutableDictionary,
                    let start = tcf?.tc(for: dict["Start"] as? String),
                    let end = tcf?.tc(for: dict2["End"] as? String),
                    let startFrs = tcc?.tc(toBinary: start, withType: 3),
                    let endFrs = tcc?.tc(toBinary: end, withType: 3),
                    tc >= startFrs,
                    tc < endFrs{
